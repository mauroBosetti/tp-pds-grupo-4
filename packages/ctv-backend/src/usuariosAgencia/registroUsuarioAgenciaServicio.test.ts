...
const crearUsuarioAgencia = vi.mocked(cliente.usuarioAgencia.create)
...
describe('registrarUsuarioAgencia', () => {
  ...
  it('falla cuando faltan datos', async () => {
    ...
    expect(crearUsuarioAgencia).not.toHaveBeenCalled()
  })
  ...
  it('crea el usuario asociándolo a la agencia del código', async () => {
    ...
    crearUsuarioAgencia.mockResolvedValue({ id: 'u-1', nombre: 'Bruno', cuentaId: 'c-1', agenciaId: 'ag-1' })

    await registrarUsuarioAgencia(datosValidos)

    const argumentos = crearUsuarioAgencia.mock.calls[0][0]
