...
export type RolUsuario = 'agencia' | 'cliente'
...
export async function loginAgencia(email: string, clave: string): Promise<ResultadoLoginUsuario> {
  const respuesta = await fetch(`${API_URL}/api/auth/agencia/login`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ email, clave }),
  })
...
export async function loginCliente(email: string, clave: string): Promise<ResultadoLoginUsuario> {
  const respuesta = await fetch(`${API_URL}/api/auth/cliente/login`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ email, clave }),
  })
